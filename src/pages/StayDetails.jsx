        ...
        <div className="middle-section">
          <div className="main-content">
            <div className="place-details">
              ...
              <div className="rate">★ 5.0 · {stay.reviews?.length || 0} reviews</div>
            </div>
              ...
              <div className="amenities-grid">
                {amenitiesToShow.map((amenity, idx) => (
                  <div key={idx} className="amenity-item">
                    <span className="amenity-icon" dangerouslySetInnerHTML={{ __html: amenity.svg }}></span>
                    <span className="amenity-label">{amenity.name}</span>
                  </div>
                ))}
              </div>
              {stay.amenities.length > 10 && (
                <button className="show-all-btn" onClick={() => setShowAllAmenities(prev => !prev)}>
                  {showAllAmenities
                    ? "Hide some amenities"
