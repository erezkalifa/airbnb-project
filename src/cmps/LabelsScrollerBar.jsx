...
export function LabelsScrollerBar({selectedLabel, onLabelSelect}){
    ...
    return(
        <div className={`labels-scroller-wrapper ${!showArrows ? 'no-arrows' : ''}`}> 
            <Carousel
            ...
            >
                {labels.map((label , idx) => (
                    <div className={`carousel-item ${selectedLabel === label.name ? 'active' : ''}`} 
                    key={idx} onClick={() => onLabelSelect(label.name)}
                    >
